...
namespace VSharp::Frontend
{
	class Lexer
	{
		...
		public:
			...
			explicit Lexer(std::u16string source) :
		...
		public:
			...
			[[nodiscard]] static constexpr System::Char16 InvalidChar() { return -1; }

			// The current character the lexer is examining, decimals, numbers, identifiers, etc...
			[[nodiscard]] System::Char16 Current() const { return Peek(0); }
			// The next token in line for the Lexer, this always points 1 position AHEAD of Current()
			[[nodiscard]] System::Char16 Next() const { return Peek(1); }
		...
		public:
			...
			[[nodiscard]] static std::vector<Syntax::SyntaxToken> CollectTokens(const std::u16string& source);
		...
		private:
			...
			[[nodiscard]] System::Char16 Peek(const System::Int32 offset = 0) const; // offset is signed because sometimes we'll need to peek backwards

			// Advances 1 in the source text and returns the character the Lexer is currently sitting on after advancing
			System::Char16 Advance();
			...
			[[nodiscard]] std::u16string GetFullTokenText() const;
			...
			void ValidateDecimalLiteral(const std::u16string& text);
			void ValidateIntegerLiteral(const std::u16string& text);
